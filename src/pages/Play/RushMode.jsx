...
const [popup, setPopup] = useState({ visible: false, title: "", message: "", buttons: [] });
  ...
  const closePopup = () => setPopup({ visible: false, title: "", message: "", buttons: [] });
  // Game refs
...
return true;
  ...
  const saveScore = async (score) => {
    ...
    // 1. Username / UID missing
    if (!uid || !username) {
      setPopup({
        visible: true,
        title: "⚠️ Login Required",
        message: "You need to login to save your score.",
        buttons: [
          { text: "Login", action: () => navigate("/premium") },
          { text: "Not now", action: closePopup }
        ]
      });
      return;
    }
    ...
    if (!premium) {
      setPopup({
        ...
        message: "Only premium users can save scores.",
        buttons: [
          { text: "Buy Premium", action: () => navigate("/premium") },
          { text: "Not now", action: closePopup }
        ]
      });
      return;
    ...
    setPopup({
      visible: true,
      title: "⏳ Saving...",
      message: "Your score is being saved.",
      buttons: []
    });

    try {
      const res = await fetch("https://score-bac.onrender.com/scores/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ uid, username, premium, score })
      });
      ...
      setTimeout(() => closePopup(), 2000); // auto-close after 2s
    } catch (e) {
      setPopup({
        visible: true,
        title: "❌ Failed",
        message: "Failed to save score: " + e.message,
        buttons: []
      });
...
>
  ...
  <div className="popup">
    <div className="popup-content">
      ...
      <h2>⚠️ Premium Required</h2>
      <p>You must be a premium user to save score & join tournaments.</p>
      <div className="popup-buttons">
        <button onClick={() => navigate("/premium")} className="yes-btn">
          Go Premium
        </button>
        <button onClick={() => setPremiumError(false)} className="no-btn">
          Cancel
        </button>
      </div>
    </div>
          ...
          <div className="popup-box">
            ...
            <div className="popup-actions">
              {popup.buttons.map((btn, i) => (
                <button key={i} onClick={btn.action}>{btn.text}</button>
              ))}
            </div>
          </div>
        </div>
